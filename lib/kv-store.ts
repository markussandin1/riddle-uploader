...
const FEED_ITEMS_KEY = 'rss:feed-items';
const SCHEDULED_JOBS_KEY = 'rss:scheduled-jobs';
const CREATED_QUIZZES_KEY = 'riddle:created-quizzes';
...
// Created Quizzes
export const loadCreatedQuizzes = async (limit: number = 20): Promise<CreatedQuiz[]> => {
  try {
    const quizzes = await kv.get<CreatedQuiz[]>(CREATED_QUIZZES_KEY);
    if (!quizzes) return [];
    // Sort by timestamp descending and respect limit
    return quizzes
      .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
      .slice(0, limit);
  } catch (error) {
    console.error('Error loading created quizzes from KV:', error);
    return [];
  }
};

export const addCreatedQuiz = async (quiz: Omit<CreatedQuiz, 'id' | 'timestamp'>): Promise<CreatedQuiz> => {
  ...
  try {
    const existingQuizzes = await loadCreatedQuizzes(100); // Load more to avoid growing list indefinitely
    const updatedQuizzes = [newQuiz, ...existingQuizzes];
    const limitedQuizzes = updatedQuizzes.slice(0, 50); // Keep last 50
    
    await kv.set(CREATED_QUIZZES_KEY, limitedQuizzes);
    return newQuiz;
  } catch (error) {
    console.error('Error adding created quiz to KV:', error);
    throw error;
  }
};
